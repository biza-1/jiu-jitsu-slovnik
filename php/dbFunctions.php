<?php
    function getSlovickaRequest() { // for getting techniques and word form MYSLQ || BIZA
        global $db;
        // getting all by JOIN
        $sql = "SELECT slovicka.ID, slovicka.japanese, slovicka.czech, slovicka.type, techniky.imageUrl, techniky.content FROM slovicka LEFT JOIN `techniky` ON slovicka.ID=techniky.ID ORDER By japanese ASC";
