function showSearchResult(data) {
    ...
    if ($('#japan_czech').text() == 'CZ') {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            output.push(
                '<div class="z-depth-1 searchResultDiv" data-id="' +
                    data[i][IDIndexName] +
                    '">',
            );
            output.push(
                '<div class=""><p class=" flow-text searchINByJQUERY">' +
            ...
            if (data[i][typeIndexName] == 'word') {
                output.push(
                    '<div class=""><p class=" flow-text">slovíčko</p></div>',
                );
            } else {
                output.push(
                    '<div class=""><p class=" flow-text">' +
...
function showTechniquesToSearch(data) {
    ...
    output += '<label>';
    output +=
        '<input type="checkbox" checked="checked" id="selectAllDiselectAllController" class="TechniquesWordsOnClick" />';
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        output += '<label>';
        output +=
            '<input type="checkbox" checked="checked" class="selectAllDiselectAll TechniquesWordsOnClick" data-searchvalue="' +
...
function showModalStitkyController(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        output +=
            '<div class="modalStitkyControleerContainer"><p class=" flow-text">' +
            data[i][nameIndexNameLABELS] +
            '</p>';
        output +=
            '<a class="btn-floating btn-small waves-effect waves-light red right buttonsToAddRemove modalStitkyAdd" data-id="' +
            data[i][IDIndexNameINLABELS] +
            '" data-name="' +
            data[i][nameIndexNameLABELS] +
            '" ><i class="material-icons">add</i></a>';
        output +=
            '<a class="btn-floating btn-small waves-effect waves-light red right buttonsToAddRemove modalStitkyRemove" data-id="' +
            data[i][IDIndexNameINLABELS] +
            '" data-name="' +
            data[i][nameIndexNameLABELS] +
...
function showModalStitkyControllerSTITKY(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        output +=
            ...
            '">';
        output +=
            '<p class=" flow-text">' + data[i][nameIndexNameLABELS] + '</p>';
        output += '</a>';
        output +=
...
function showSearchResultBYID(data, StitkyData) {
    ...
    getAndApeendNamesStitky().then(function(databata) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            output += '</div>';
            output +=
                '<a class="btn-floating btn-small waves-effect waves-light red right modalStitkyRemove" data-id="' +
