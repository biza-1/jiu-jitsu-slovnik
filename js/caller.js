$(document).ready(function() {
  ...
  $("#searchResult").on("click", ".searchResultDiv", function(e) {
    if (
      ...
      if ($("#searchResult").find("div.selectedSearchResult").length === 0) {
  ...
  // section for STITKY
  $("#StitkyModalOpener").on("click", function(e) { // when User click <a> to open Modal to change Stitky    
    ifUserLogedIn().then(function(data) {
      // for checking if logged in indexedDB
      if (data === true) {
        showModalStitkyAdder();
        getModalStitkyController(); // shows all Stitky
      } else {
        $("#LabelAdd").html("pro tuto funkci musi byt uzivatel prihlasen"); // when user is not signed in > message        
      }
    });
  });
  ...
  $('#LabelAdd').on("click", "#pridatStitekButton",  function(e) { 
    ...
    checkIfStitekIsInDB(nazevStitku).then(function(isInDB) {
      if (nazevStitku === '') {
        ...
        $('#StitkyMessage').html('Štítek už je v databázi');
      } else {
        ...
        checkStitkyAmmount().then(function(stitkyAmmount) {
          if (stitkyAmmount <= maxAmmountStitky) {
            ...
            $('#StitkyMessage').html('Dosáhl jsi maximálního počtu štítků (' + maxAmmountStitky + ').');
          }
        });
      }
    });
  ...
  $('#LabelAdd').on("click", ".modalStitkyAdd", function(e) {
    ...
    // to get all oznacene
    var vsechnyOznacene = $('.selectedSearchResult').map(function() {
      return $(this).data('id');
    }).get();
    var nameOfTehcniques = $(this).text();
  ...
  $('#LabelAdd').on("click", ".modalStitkyRemove", function(e) {
    ...
    removeLabelFromWord(stitekID, vsechnyOznacene);
  });
