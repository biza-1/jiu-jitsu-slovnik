$(document).ready(function() {
  ...
  $("#modalStitkyConrolerrDiv").on("click", ".modalStitkyEdit", function() {
    ...
    var output =
      '<input id="showName' +
      whereTochange +
      '" type="text" style="border:1px solid rgb(196, 196, 196); margin-left:8px; width:190px;margin-top:7px" class="validate input_css w3-bar-item cahngeStitkyValue' +
      whereTochange +
    ...
    output +=
      '<a class="w3-button w3-bar-item right buttonsToAddRemove modalStitkyNotOk" style="margin-right:5px;" data-id="' +
      whereTochange +
      '" data-name="' +
      nameOfStitky +
      '"><i class="material-icons">clear</i></a>';
    output +=
      '<a class="w3-button w3-bar-item right buttonsToAddRemove modalStitkyOK" data-id="' +
      whereTochange +
      '" data-name="' +
      nameOfStitky +
  ...
  $("#modalStitkyConrolerrDiv").on("click", ".modalStitkyNotOk", function() {
    ...
    output += '<div class="w3-bar-item">' + nameOfStitky + "</div>";
    output += "</a>";
    output +=
          ...
          if (isInDB === true) {
            ...
            renameStitekInDB(whereTochange, newName, nameOfStitky).then(
              function() {
                ...
                output += '<div class="w3-bar-item">' + newName + "</div>";
                output += "</a>";
                output +=
                  '<a class="w3-bar-item w3-button right buttonsToAddRemove modalStitkyDelete" style="margin-right:5px;" data-id="' +
                  whereTochange +
